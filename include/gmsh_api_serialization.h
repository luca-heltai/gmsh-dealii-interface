...
namespace GMSH
{
    ...
    const std::string &file_suffix = ".msh")
  {
    ...
    std::string fname = file_prefix + "_" + std::to_string(rank + 1) + file_suffix;
    ...
    std::vector<std::vector<dealii::TriangulationDescription::CellData<dim>>> cell_infos(1);
    ...
    for (unsigned int r = 0; r < nprocs; ++r)
    {
      if (rank == r)
      {
        ...
        std::cout << "Rank " << rank << " opening mesh: " << fname << std::endl;
        gmsh::open(fname);
              ...
              for (unsigned int j = 0; j < element_ids[i].size(); ++j)
              {
                ...
                for (unsigned int v = 0; v < n_vertices; ++v)
                {
                  ...
                  AssertThrow(node_tag_to_index.find(node_tag) != node_tag_to_index.end(),
                              dealii::ExcMessage("Node tag " + std::to_string(node_tag) + " not found in node list!"));
                ...
                dealii::TriangulationDescription::CellData<dim> cell_info;
                cell_info.id =
                  dealii::CellId(element_ids[i][j], {}).template to_binary<dim>();
    ...
    triangulation_description.coarse_cell_index_to_coarse_cell_id = coarse_cell_ids;
